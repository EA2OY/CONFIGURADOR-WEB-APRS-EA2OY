...
## Usage / Uso
1. Open `configurador.html` with **desktop Chrome or Edge** (WebSerial is not available in Firefox/Safari).
   Abre `configurador.html` con **Chrome o Edge de escritorio** (WebSerial no está en Firefox/Safari).
...
`configurador.html`** (so it cannot go stale), runs it against 18 configurations and checks
...
`prueba_avisos.js` coge el código de avisos **tal cual está en `configurador.html`** (así no
